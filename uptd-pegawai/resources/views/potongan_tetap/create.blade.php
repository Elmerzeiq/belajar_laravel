...
@section('content')
    <div class="container">
        ...
        <form
            ...
            {{-- Nama Potongan --}}
            <div class="mb-3">
                <label for="nama_potongan" class="form-label @error('nama_potongan') text-danger @enderror">Nama
                    Potongan</label>
            ...
            <div class="mb-3">
                ...
                <select name="jenis_potongan" id="jenis_potongan"
                    class="form-select @error('jenis_potongan') is-invalid @enderror">
                    <option value="gaji_pokok"
                        {{ old('jenis_potongan', $potongan_tetap->jenis_potongan ?? '') == 'gaji_pokok' ? 'selected' : '' }}>
                        Gaji Pokok</option>
                    <option value="insentif"
                        {{ old('jenis_potongan', $potongan_tetap->jenis_potongan ?? '') == 'insentif' ? 'selected' : '' }}>
                        Insentif</option>
                    <option value="total"
                        {{ old('jenis_potongan', $potongan_tetap->jenis_potongan ?? '') == 'total' ? 'selected' : '' }}>
                        Total (Gaji Pokok + Insentif)</option>
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="tipe" id="tipe-potongan" class="form-select @error('tipe') is-invalid @enderror"
                    ...
                    <option value="tetap" {{ old('tipe', $potongan_tetap->tipe ?? '') == 'tetap' ? 'selected' : '' }}>Tetap
                        (Rp)</option>
                    <option value="persen" {{ old('tipe', $potongan_tetap->tipe ?? '') == 'persen' ? 'selected' : '' }}>
                        Persen (%)</option>
                </select>
                @error('tipe')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    <span
                        class="input-group-text satuan">{{ old('tipe', $potongan_tetap->tipe ?? '') == 'persen' ? '%' : 'Rp' }}</span>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            jumlahInput.addEventListener('input', function() {
                ...
                if (tipe === 'persen') {
                    ...
                    const parts = jumlahInput.value.split('.');
                    if (parts.length > 2) {
                        jumlahInput.value = parts[0] + '.' + parts[1];
                    }

                    // Jangan proses jika hanya titik atau kosong
                    if (jumlahInput.value === '' || jumlahInput.value === '.') return;

                    let val = parseFloat(jumlahInput.value);
                    if (!isNaN(val)) {
                        if (val > 100) jumlahInput.value = 100;
                        else if (val < 0) jumlahInput.value = 0;
                    }
                }
            });
        });
